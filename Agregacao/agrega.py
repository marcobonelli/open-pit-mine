...
def agregaIJ(listaBlocos, i, j):
	...
	listaBlocos[i].agregados = Union(listaBlocos[i].agregados , listaBlocos[j].agregados)
	...
	for v in listaBlocos[j].listaP:
		if v !=i:
			if j in listaBlocos[v].listaSucessor:
				listaBlocos[v].listaSucessor.remove(j) # remove j da lista de predecessores de seu sucessor
			listaBlocos[v].listaSucessor = Union(listaBlocos[v].listaSucessor, [i]) # adiciona o i na lista de predecessores do sucessor de j
			listaBlocos[i].listaP = Union(listaBlocos[i].listaP,[v]) # adiciona o sucessor de j na lista de sucessores de i

	for v in listaBlocos[j].listaSucessor:
		if v !=i:
			listaBlocos[v].listaP.remove(j) # remove j da lista de predecessores de seu sucessor
			listaBlocos[v].listaP = Union(listaBlocos[v].listaP, [i]) # adiciona o i na lista de predecessores do sucessor de j
			listaBlocos[i].listaSucessor = Union(listaBlocos[i].listaSucessor , [v]) # adiciona o sucessor de j na lista de sucessores de i
...
def criaListaBlocosAgregados(listaBlocos,cap):

	lB = copy.deepcopy(listaBlocos)
	...
	while (change):
		...
		for i in lB:
			if i.excluido == 0:
				while True:
					Cand = i.candAgregacao(lB,cap)
					if Cand >= 0:
						change =1
						agregaIJ(lB, i.id, Cand)
					else:
						break
	# for b in lB:
	# 	print(b)
	[rLista,mapB] = listaAgregada(lB)
	return [rLista,mapB]

def listaAgregada(listaBlocos):
	rLista = []
	ni = 0
	mapB = dict()
	for i in listaBlocos:
		if not i.excluido:
			rLista.append(i)
			mapB[i.id] = ni
			ni = ni+1
...
if __name__=="__main__":
	...
	[agLista,mapB] = criaListaBlocosAgregados(B,4)
	for i in range(len(agLista)):
		print(agLista[i])
	print(mapB)
	#print (dfsPath(B,17,8,start = 1))
	# print (B[11].candAgregacao(B,4))
	# agregaIJ(B, 11, 2)
	# print(B[11])
	# print(B[2])
	# print (B[11].candAgregacao(B,4))
...
# Falta olhar questao da repeticao dos blocos nas listas de sucessores e predecessores
